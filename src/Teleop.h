	...
	//      else if((REQ_POS[i])<(dist-0.05))
	   ...
	   {
			ROS_INFO("I am near my destination hence called");
	       joy_sub_ = nh_.subscribe<sensor_msgs::Joy>("/joy", 100, &TeleopTurtle::joyCallback, this);
	//	if((Q==7)||(Q==9)||(Q==13)||(Q==15))
	//	{
		   // performance function passing distance 
	    } 
...
/*	  if((joy->buttons[0]==1)&&(joy->buttons[3]==1))
	  {
	     ros::Duration(2.0).sleep();
	     fptr w = functioncall(chosenfunction,double(    dist));
	     if(w()>compare)
	     {
