...
void Tele_current::joyCall(const sensor_msgs::Joy::ConstPtr& wii_call)
{
  ...
  //if(flag==0 && ((laser_Wii.ranges[0])<=2.3))
    ...
    {
      vel = drive_fb(-0.5*wii_call->buttons[8], vel, dist);
    }
    else if(wii_call->buttons[9] == 1)
    {
      vel = drive_fb(0.5*wii_call->buttons[9], vel, dist);
    }
    else
    {
      vel = drive_fb(0, vel, dist); 
    }
  ...
  if(perf_dist<randomized)
    {
      ...
      //ros::Duration(2.0).sleep();
      flag = 1;
      //joy_sub_ = nh_.subscribe<sensor_msgs::Joy>("/joy", 100, &Tele_current::joyCallback, this);
    }
  else
    {
